...
3. Keep the demo app modern and lightweight.
...
4. Verify current Cloudflare documentation before platform changes.
...
5. Use the session layer before adding UI-local transport state.
...
7. Keep auth and pairing explicit.
...
- `packages/channel-contract/`
  Shared protocol types and route helpers.
- `packages/channel-client/`
  Headless client SDK and session helper.
- `packages/openclaw-channel/`
  Native OpenClaw plugin package.
- `apps/web-demo/`
  Reference UI built on the client SDK.
